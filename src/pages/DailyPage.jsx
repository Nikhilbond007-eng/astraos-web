...
import { getDailyReading } from '../utils/api'
...
export function DailyPage() {
  ...
  useEffect(() => {
    ...
    const generate = async () => {
      try {
        const result = await getDailyReading(chart, userName)
        setReading(result)
      } catch {
  ...
  if (!chart) return (
    <main style={{ position: 'relative', zIndex: 2, paddingTop: 80 }}>
      <div className="section-wrap-sm" style={{ textAlign: 'center', paddingTop: 120 }}>
        ...
        <p style={{ color: 'var(--muted)', fontSize: 17, fontStyle: 'italic', marginBottom: 32 }}>
          Generate your birth chart first to receive your personalised daily reading.
        </p>
  ...
  return (
    <main style={{ position: 'relative', zIndex: 2, paddingTop: 80 }}>
      <div className="section-wrap">
        ...
        <p style={{ color: 'var(--muted)', fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: 2, marginBottom: 40 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 24 }}>
            ...
            <div className="card" style={{ padding: 32, background: 'linear-gradient(135deg,rgba(212,168,83,.06),rgba(124,92,252,.04))', borderColor: 'rgba(212,168,83,.2)' }}>
              ...
              {loading ? (
                <div style={{ textAlign: 'center', padding: '32px 0' }}>
                  ...
                  <p style={{ marginTop: 16, color: 'var(--muted)', fontStyle: 'italic', animation: 'pulse 1.5s infinite' }}>
                ...
                <div style={{ fontSize: 17, lineHeight: 1.9, color: 'var(--muted)', fontStyle: 'italic' }}>
                  {reading.split('\n').map((line, i) =>
                    line ? <p key={i} style={{ marginBottom: 14 }}>{line}</p> : null
                  )}
                </div>
              )}
            </div>
            ...
            <div className="card">
              ...
              {transits.map((t, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: i < transits.length - 1 ? '1px solid rgba(255,255,255,.04)' : 'none' }}>
                  ...
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, padding: '3px 8px', borderRadius: 6, background: `${typeColor[t.type]}18`, border: `1px solid ${typeColor[t.type]}40`, color: typeColor[t.type], whiteSpace: 'nowrap' }}>
                    {typeLabel[t.type]}
                  </span>
                </div>
              ))}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div className="card" style={{ textAlign: 'center', padding: 28 }}>
              ...
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: 2, color: 'var(--muted2)', marginBottom: 14 }}>
                {mp.pct}% Illuminated
              </div>
              ...
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, letterSpacing: 2, color: 'var(--gold)', opacity: .6, marginBottom: 8 }}>
                TONIGHT'S RITUAL
              </div>
            ...
            <div className="card">
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
                {chart.luckyNums.map(n => (
                  <div key={n} style={{ width: 44, height: 44, borderRadius: '50%', border: '1px solid rgba(212,168,83,.3)', background: 'rgba(212,168,83,.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-serif)', fontSize: 16, color: 'var(--gold)' }}>
                    {n}
                  </div>
                ))}
              </div>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {[['#d4a853','Gold'],['#7c5cfc','Indigo'],['#2dd4c0','Teal']].map(([c, name]) => (
                  <div key={name} style={{ flex: 1, height: 32, borderRadius: 8, background: c, opacity: .75, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-mono)', fontSize: 9, color: c === '#d4a853' ? '#000' : '#fff' }}>
                    {name}
                  </div>
                ))}
              </div>
            ...
            <div className="card" style={{ padding: 20 }}>
              ...
              <p style={{ fontSize: 14, color: 'var(--muted)', marginBottom: 14 }}>
                Have a specific question about today's energies? Ask the AI.
              </p>
              <Link to="/chart" className="btn-secondary" style={{ display: 'block', textAlign: 'center', width: '100%' }}>
                Open Astro Chat →
              </Link>
            </div>
          </div>
